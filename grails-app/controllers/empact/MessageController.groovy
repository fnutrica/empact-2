...
class MessageController {
    ...
    def list(Integer max) {
        ...
        if (conversationInstance && (!conversationInstance.isRead && !conversationInstance.owner.toString().equals(session.user.toString()))) {
    ...
    def suggestUsers(String query) {
        query = "%" + query + "%"
        def users = EndUser.findAllByFirstNameIlikeOrLastNameIlike(query, query)
        def user_list = new ArrayList<String>()

        for (EndUser user : users) {
            user_list.add(["label": user.toString(), "id": user.id])
        }

        render([options: user_list] as JSON)
    }
